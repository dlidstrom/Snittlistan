...
module Bits =
    type IClient =
        abstract member GetMatchScores :
            Domain.MatchId ->
            Async<Contracts.Bits.MatchScores.MatchScore>

        abstract member GetMatchResults :
            Domain.MatchId ->
            Domain.MatchSchemeId ->
            Async<Contracts.Bits.MatchResults.MatchResult>

        abstract member GetDivision :
            Domain.SeasonId ->
            Async<Contracts.Bits.Divisions.Division array>

        abstract member GetMatch :
            Domain.DivisionId ->
            Domain.SeasonId ->
            Async<Contracts.Bits.Match.Match array>
    ...
    type Client(apiKey, http: Contracts.RequestStringAsync) =
        ...
        interface IClient with
            ...
            member _.GetMatchScores(Domain.MatchId matchId) =
                let url =
                    $"%s{root}/matchResult/GetMatchScores?APIKey=%s{apiKey}&matchId=%d{matchId}"

                let matchScoresResponse =

                    async {
                        let! r = http { defaultRequest with Url = url }
            ...
            member _.GetMatchResults (Domain.MatchId matchId) (Domain.MatchSchemeId matchSchemeId) =
                let url =
                    $"%s{root}/matchResult/GetMatchResults?APIKey=%s{apiKey}&matchSchemeId=%s{
                                                                                                  matchSchemeId
                    }&matchId=%d{matchId}"
            ...
            member _.GetMatch (Domain.DivisionId divisionId) (Domain.SeasonId seasonId) =
                let url =
                    $"%s{root}/Match/?APIKey=%s{apiKey}&divisionId=%d{divisionId}&seasonId=%d{
                                                                                                  seasonId
                    }"
