    ...
    public static SummaryPattern[] Create(Dictionary<string, Player> players)
    {
        ...
        SummaryPattern[] summaryPatterns = new[]
        {
                ...
                new SummaryPattern("[9-13]-x win")
                {
                    ...
                    Commentary = seriesScores =>
                    {
                        SeriesScores lastLastLastSeries = seriesScores[seriesScores.Length-3];
                        SeriesScores lastLastSeries = seriesScores[seriesScores.Length-2];
                        SeriesScores lastSeries = seriesScores.Last();
                        ...
                        SeriesScores greatSeries = seriesScores.FirstOrDefault(x => x.TeamScoreDelta == 5 && x.TeamScoreTotal <= 10);
                        if (greatSeries != null)
                        {
                            sentences.Add(
                        ...
                        {
                            IEnumerable<PlayerResult> winningPlayers = lastSeries.PlayerResults.Where(x => x.Score > 0);
                            string nicknames = string.Join(
                                " och ",
                                winningPlayers.Select(x => players[x.PlayerId].Nickname));
                            string sentence = $"{nicknames} avgjorde matchen med en stark insats i sista serien.";
                ...
                new SummaryPattern("all 4 series losses")
                {
                    ...
                    Commentary = seriesScores =>
                    {
                        ...
                        SeriesScores lossSeries = seriesScores.Take(3)
                                                     .Skip(1)
                        ...
                        if (lossSeries != null)
                        {
                            string sentence = $"Motståndarna lade grunden till vinsten genom att göra {lossSeries.FormattedInvertedDeltaResult} i serie {lossSeries.SerieNumber} vilket resulterade i poängställningen {lossSeries.FormattedResult}.";
                            sentences.Add(sentence);
                        }
